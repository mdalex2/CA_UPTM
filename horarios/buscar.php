...
<form id="form_buscar" name="form_buscar" action="<?php echo "horarios.php?accion=buscar" ?>" method="post" enctype="multipart/form-data" class="uniForm">
<fieldset>
...
$("#txt_buscar").focus();'> 
<OPTION VALUE="horarios.cedula" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="horarios.cedula") echo " selected ";?>>Nº C&Eacute;DULA</OPTION>
<OPTION VALUE="turno" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="turno") echo " selected " ?>>TURNO</OPTION>
</SELECT>
...
<td width="85">  
<button formaction="<?php echo "horarios.php?accion=buscar" ?>" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="submit">
...
<?php 
	if (isset($_POST["txt_buscar"]) && isset($_POST["cmb_tip_bus"])){
		...
		$consulta=ejecuta_sql("select horarios_id,horarios.Cedula,personal.Nombre,personal.Apellido,turno,Hora_De_Entrada,Hora_De_Salida FROM (horarios 
		INNER JOIN personal on horarios.Cedula=personal.Cedula
		) ORDER BY personal.Nombre,personal.Apellido ASC",true);} else {
		$consulta=ejecuta_sql("select horarios_id,horarios.Cedula,personal.Nombre,personal.Apellido,turno,Hora_De_Entrada,Hora_De_Salida FROM (horarios 
		INNER JOIN personal on horarios.cedula=personal.cedula
		) where  $campo_buscar LIKE '%$texto_buscar%' ORDER BY personal.Nombre,personal.Apellido ASC",true);} 
		
		if ($consulta)
			{
				if (mysql_num_rows($consulta)==1) {
					$fila=mysql_fetch_array($consulta);
					$url="horarios.php?accion=mostrar&id_mos=".$fila['horarios_id'];
					echo '
...
<table id="tablasinbtn_desc" class="letra_16 mouse_hover" width="..."> 
        <thead> 
          <tr>
            ...
            <th title="Opciones" width="80px">TURNO</th>
            <th title="Opciones" width="80px">H. ENTRADA</th>
            <th title="Opciones" width="80px">H. SALIDA</th>
        ...
        <?php
          ...
          <tr>
            ...
            <td align="center"><?php echo $fila["turno"]?></td>
            <td align="center"><?php echo formato_fecha("H12",$fila["Hora_De_Entrada"])?></td>
            <td align="center"><?php echo formato_fecha("H12",$fila["Hora_De_Salida"])?></td>
            ...
            <td align="center">
 			<a id="resize" href="horarios.php?<?php echo "accion=mostrar&id_mos={$fila['horarios_id']}";?>" class="boton" style="font-size:10px;padding:2px; color:#000000;" width="45%" height="90%" title="Mostrar" >&nbsp;
