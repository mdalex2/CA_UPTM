...
$dias = new Dias();
aplica_conf_global();
if (isset($_POST["txt_desde"]) & isset($_POST["txt_hasta"])){
	$fecha_ini=date("Y-m-d",strtotime($_POST["txt_desde"]));
	$fecha_fin=date("Y-m-d",strtotime($_POST["txt_hasta"]));
} else {
	$fecha= date("Y-m-d");
...
<table width="100%" border="0">
  <tr>
    <td align="center"><span class="titulo">REPORTE DE ASISTENCIA PERSONAL</span><strong><span class="titulo"> (consulta individual)&nbsp;<?php echo "DESDE: ".date("d-m-Y",strtotime($fecha_ini))." HASTA: ".date("d-m-Y",strtotime($fecha_fin));?></span></strong></td>
  </tr>
</table>
...
//echo $fecha_ini." - ".$fecha_fin."usu: ".$cedula;
  $sql="select asistencias.*,personal.Nombre,personal.Apellido, personal.cargo,horarios.turno,horarios.Hora_De_Entrada,horarios.Hora_De_Salida FROM (asistencias 
		INNER JOIN personal on asistencias.Cedula=personal.Cedula
		INNER JOIN horarios on horarios.Cedula=asistencias.Cedula
		) WHERE asistencias.Cedula='$cedula' and Fecha between DATE('$fecha_ini') and DATE('$fecha_fin') ORDER BY Fecha ASC";
  $db=conectarse();
