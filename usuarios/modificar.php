...
function modificar_datos(){
	if (empty($_POST)){ //si no se han recibido datos muestro error
			...
			$db=conectarse();
			$consulta_sql=mysql_query("UPDATE seguridad set  	cedula_usu='$cedula',
			nombre_usuario='$nombre',
			apellido_usuario='$apellido',
			usuario='$login',
			clave='$password',
			privilegio='$privilegio' where cedula_usu='$id_ant'");
			if (!$consulta_sql){
				  $error=obtener_error(mysql_errno());
					$botones= mostrar_btn_imp_reg();
					mostrar_box("err",false,"INFORMACIÓN","No se pudo guardar el usuario en la base de datos: <br>".$error);	
